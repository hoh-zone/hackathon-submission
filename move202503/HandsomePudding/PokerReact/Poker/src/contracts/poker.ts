...
import {BALANCE_EVENT, RANDOM, GET_BALANCE, ADD_BALANCE, PLAY_GAME,GAME_STORE, GAME_RESULT_EVENT} from "../config/key";
...
export async function playGame(signAndExecuteTransaction: any, bet_a: number, bet_b: number, bet_c: number, bet_d: number, bet_e: number) {
    const tx = new Transaction();
    ...
    tx.moveCall({
        ...
        arguments: [
            tx.object(GAME_STORE),
            tx.object(RANDOM),
            tx.pure.u64(scaled_bet_a),
            tx.pure.u64(scaled_bet_b),
            tx.pure.u64(scaled_bet_c),
            tx.pure.u64(scaled_bet_d),
            tx.pure.u64(scaled_bet_e)
        ],
    });
    const response = await signAndExecuteTransaction({transaction: tx});
    console.log("Full response:", response);
    ...
    response.events?.forEach((event: any) => {
        ...
        if (event.type === GAME_RESULT_EVENT) {
            ...
            gameResult.final_balance = Number((gameResult.final_balance / 10).toFixed(1));
            gameResult.prize = Number((gameResult.prize / 10).toFixed(1));
